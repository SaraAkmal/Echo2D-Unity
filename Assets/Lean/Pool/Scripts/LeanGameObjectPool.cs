...
namespace Lean.Pool
{
    ...
    public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        private static readonly Dictionary<GameObject, LeanGameObjectPool> prefabMap =
        ...
        /// <summary>
        ///     This allows you to control how spawned/despawned GameObjects will be handled. The <b>DeactivateViaHierarchy</b>
        ///     mode should be used if you need to maintain your prefab's de/activation state.
        ...
        /// </summary>
        private readonly HashSet<GameObject> spawnedClonesHashSet = new HashSet<GameObject>();
        ...
        /// <summary>
        ///     If you're using the <b>Strategy = DeactivateViaHierarchy</b> mode, then all despawned clones will be placed
        ///     under this.
        /// </summary>
...
#if UNITY_EDITOR
        /// <summary>
        ///     This will return false if you have pre-loaded prefabs do not match the <b>Prefab</b>.
        ...
        public bool DespawnedClonesMatch
        {
            get
            {
                for (var i = despawnedClones.Count - 1; i >= 0; i--)
                {
                    ...
                    if (clone != null && PrefabUtility.GetCorrespondingObjectFromSource(clone) != prefab) return false;
                }
            ...
            Transform parent, bool worldPositionStays)
        {
            if (prefab != null)
            {
                ...
                if (Capacity <= 0 || Total < Capacity)
                {
                    ...
                    // Activate?
                    if (Strategy == StrategyType.ActivateAndDeactivate) clone.SetActive(true);
        ...
        public void DespawnAll()
        {
            ...
            for (var i = spawnedClonesList.Count - 1; i >= 0; i--)
            {
                ...
                if (clone != null) DespawnNow(clone);
            }

            spawnedClonesList.Clear();

            // Clear all delays
            for (var i = delays.Count - 1; i >= 0; i--) LeanClassPool<Delay>.Despawn(delays[i]);
            ...
            if (clone != null)
            {
                ...
                else
                {
                    ...
                    for (var i = delays.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (delay.Clone == clone) delays.RemoveAt(i);
                    }
                }
            }
            else
        ...
        /// <summary>
        ///     This will destroy all preloaded or despawned clones. This is useful if your prefab has been modified, and the
        ///     old ones are no longer useful.
        /// </summary>
        [ContextMenu("Clean")]
        public void Clean()
        {
            for (var i = despawnedClones.Count - 1; i >= 0; i--) DestroyImmediate(despawnedClones[i]);
        ...
        private void RegisterPrefab()
        {
            if (prefab != null)
            {
                ...
                if (prefabMap.TryGetValue(prefab, out existingPool))
        ...
        private void UnregisterPrefab()
        {
            ...
            if (Equals(prefab, null)) return;

            var existingPool = default(LeanGameObjectPool);

            if (prefabMap.TryGetValue(prefab, out existingPool) && existingPool == this) prefabMap.Remove(prefab);
        }
        ...
        {
            // Add clone to despawned list
            if (register) despawnedClones.Add(clone);
        ...
        {
#if UNITY_EDITOR
            if (Application.isPlaying == false && PrefabUtility.IsPartOfRegularPrefab(prefab))
            {
                if (worldPositionStays) return (GameObject) PrefabUtility.InstantiatePrefab(prefab, parent);
            ...
            if (worldPositionStays) return Instantiate(prefab, parent, true);
            ...
            Transform parent, bool worldPositionStays)
        {
            ...
            if (parent == null) SceneManager.MoveGameObjectToScene(clone, SceneManager.GetActiveScene());
...
namespace Lean.Pool
{
    ...
    public class LeanGameObjectPool_Inspector : LeanInspector<LeanGameObjectPool>
    {
        protected override void DrawInspector()
        {
            ...
            if (Draw("prefab", "The prefab this pool controls."))
